...
let AttendanceRecord = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec4 = view(), _dec5 = call({}), _dec(_class = (_class2 = class AttendanceRecord {
  attended = new UnorderedMap('unique-id-map1');
  mark_attendance({
    student_id,
    class_id
  }) {
    const checker = predecessorAccountId();
    const attendance = new attendace_model({
      check: true,
      checker,
      class_id
    });
    this.attended.set(student_id, attendance);
    log(`Student ${student_id} attended class with enrollment ${class_id}. Reviewed by ${checker}`);
  }
  get_all_records({
    student_id
  }) {
    ...
    if (record) {
      ...
      log(`The student ${student_id} doesn't have record`);
      return null;
    }
  }
  has_attended({
    ...
    if (attendance) {
      if (attendance.check) {
        log(`The student ${student_id} does have attendance in ${attendance.class_id}`);
      } else {
        log(`The student ${student_id} doesn't have attendance in ${attendance.class_id}`);
      }
  ...
  delete_attendance({
    ...
    if (record.check && record.class_id == class_id) {
      record.check = false;
      log(`Attendance record for student ${student_id} deleted successfully`);
      res = true;
    } else {
      log(`The student ${student_id} does not currently have assistance`);
      res = false;
    }
    return res;
  }
}, (_applyDecoratedDescriptor(_class2.prototype, "mark_attendance", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "mark_attendance"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_all_records", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "get_all_records"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "has_attended", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "has_attended"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "delete_attendance", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "delete_attendance"), _class2.prototype)), _class2)) || _class);
function delete_attendance() {
  const _state = AttendanceRecord._getState();
  if (!_state && AttendanceRecord._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = AttendanceRecord._create();
  if (_state) {
    AttendanceRecord._reconstruct(_contract, _state);
  }
  const _args = AttendanceRecord._getArgs();
  const _result = _contract.delete_attendance(_args);
  AttendanceRecord._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AttendanceRecord._serialize(_result, true));
}
function has_attended() {
  const _state = AttendanceRecord._getState();
  if (!_state && AttendanceRecord._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = AttendanceRecord._getArgs();
  const _result = _contract.has_attended(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AttendanceRecord._serialize(_result, true));
}
function get_all_records() {
  ...
  const _args = AttendanceRecord._getArgs();
  const _result = _contract.get_all_records(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AttendanceRecord._serialize(_result, true));
}
function mark_attendance() {
  ...
  const _args = AttendanceRecord._getArgs();
  const _result = _contract.mark_attendance(_args);
  AttendanceRecord._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AttendanceRecord._serialize(_result, true));
}

export { delete_attendance, get_all_records, has_attended, mark_attendance };
